...
def test_default_providers_never_scan_anything_with_an_empty_allowlist():
    ...
    result = scan_resources(allowlist=_allowlist(), scan_state=_scan_state())
...
def test_scanning_a_real_approved_tmp_folder_discovers_a_real_file(tmp_path):
    ...
    result = scan_resources(allowlist=allowlist, scan_state=_scan_state())
...
def test_a_provider_with_no_data_does_not_stop_other_providers():
    ...
    result = scan_resources(providers=[empty, working], allowlist=_allowlist(), scan_state=_scan_state())
...
def test_a_provider_that_raises_does_not_stop_other_providers():
    ...
    result = scan_resources(providers=[broken, working], allowlist=_allowlist(), scan_state=_scan_state())
...
def test_duplicate_files_are_detected_by_real_matching_hash():
    ...
    result = scan_resources(providers=[provider], allowlist=_allowlist(), scan_state=_scan_state())
...
def test_unchanged_files_are_not_reported_new_modified_or_deleted_on_the_second_scan():
    ...
    scan_resources(providers=[provider], allowlist=_allowlist(), scan_state=scan_state)
    second_result = scan_resources(providers=[provider], allowlist=_allowlist(), scan_state=scan_state)
...
def test_a_changed_hash_is_reported_modified_on_the_next_scan():
    ...
    scan_resources(providers=[first_provider], allowlist=_allowlist(), scan_state=scan_state)
    ...
    result = scan_resources(providers=[second_provider], allowlist=_allowlist(), scan_state=scan_state)
...
def test_full_real_pipeline_local_folder_provider_through_the_engine_end_to_end(tmp_path):
    ...
    first = scan_resources(allowlist=allowlist, scan_state=scan_state)
    assert str(tmp_path / "one.txt") in first["new"]

    (tmp_path / "two.txt").write_text("second")
    second = scan_resources(allowlist=allowlist, scan_state=scan_state)
